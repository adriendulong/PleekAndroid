package com.pleek.app.common;

/**
 * Created by tiago on 08/04/2015.
 */
public class Constants {

    // PREFS
    public static final String PREF_LAST_FRIENDS_UPDATE = "PREF_LAST_FRIENDS_UPDATE";
    public static final String PREF_FRIENDS = "PREF_FRIENDS";
    public static final String PREF_LAST_INBOX_UPDATE = "PREF_LAST_INBOX_UPDATE";
    public static final String PREF_LAST_SENT_UPDATE = "PREF_LAST_SENT_UPDATE";
    public static final String PREF_LAST_BEST_UPDATE = "PREF_LAST_BEST_UPDATE";

    // REQUEST
    public static final int REQUEST_ADD_USER = 0;

    // INTENT
    public static final String EXTRA_FROM_FRIENDS = "EXTRA_FROM_FRIENDS";
    public static final String EXTRA_FROM_INBOX = "EXTRA_FROM_INBOX";
}
